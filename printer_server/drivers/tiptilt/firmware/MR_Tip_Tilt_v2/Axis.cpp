...
bool Axis::moveAxis(float move_location, bool coarseMove){
    ...
    float allowedError = 1;                      //position must be within +-um of this number      
    ...
    while(abs(move_location-stepper_location) > allowedError){
        ...
        if(steps < MINIMUM_NUMBER_OF_STEPS){
            ...
            if(last_stepper_location == stepper_location){
...
float Axis::calulateLocation(){
    ...
    if(inverted){
        return encoder->read()*DISTANCE_PER_ROTATION/COUNTS_PER_ROTATION/correction_factor;
    }
    else{
        return -encoder->read()*DISTANCE_PER_ROTATION/COUNTS_PER_ROTATION/correction_factor;
    }
