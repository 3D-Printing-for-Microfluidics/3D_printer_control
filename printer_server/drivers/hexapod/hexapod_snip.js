...
function update_rotation_fields(rotation){
    ...
    // Update html elements
    document.getElementById("rotation_U_state").innerHTML = rotation[0].toFixed(1) + " &deg";
    document.getElementById("rotation_V_state").innerHTML = rotation[1].toFixed(1) + " &deg";
    document.getElementById("rotation_W_state").innerHTML = rotation[2].toFixed(1) + " &deg";
...
function update_translation_fields(translation){
    ...
    // Update html elements
    document.getElementById("translation_X_state").innerHTML = translation[0].toFixed(1) + "mm";
    document.getElementById("translation_Y_state").innerHTML = translation[1].toFixed(1) + "mm";
    document.getElementById("translation_Z_state").innerHTML = translation[2].toFixed(1) + "mm";
    
    document.getElementById("translation_X_state_input").value = parseFloat(translation[0].toFixed(1));
    document.getElementById("translation_Y_state_input").value = parseFloat(translation[1].toFixed(1));
    document.getElementById("translation_Z_state_input").value = parseFloat(translation[2].toFixed(1));
}
...
function step_axis(element){
    var command_elements = element.id.split("_");

    var command_type = command_elements[0];
    var axis = command_elements[1];
    var value = parseFloat(command_elements[2]);
...
socket.on("init_msg", function(init_output){
    ...
    var rotation = [init_pose[3], init_pose[4], init_pose[5]];
    var translation = [init_pose[0], init_pose[1], init_pose[2]];
    ...
    if (init_flag){
        ...
        // Update init status message
        var init_status_text = document.getElementById("hexapod-state");
        init_status_text.innerText = " - Initialized - ";
    }
    else{
        // Update init status message
        var init_status_text = document.getElementById("hexapod-state");
        init_status_text.innerText = " - Init failed, try again - ";
    }
});
...
send_command_btn.onclick = function(){
    // Compile inputs from all the boxes
    var translation_X_state_text = parseFloat(document.getElementById("translation_X_state_input").value);
    var translation_Y_state_text = parseFloat(document.getElementById("translation_Y_state_input").value);
    var translation_Z_state_text = parseFloat(document.getElementById("translation_Z_state_input").value);
    var rotation_U_state_text = parseFloat(document.getElementById("rotation_U_state_input").value);
    var rotation_V_state_text = parseFloat(document.getElementById("rotation_V_state_input").value);
    var rotation_W_state_text = parseFloat(document.getElementById("rotation_W_state_input").value);
...
socket.on("pose_update", function(pose){
    ...
    var translation = [pose[0], pose[1], pose[2]];
    var rotation = [pose[3], pose[4], pose[5]];
