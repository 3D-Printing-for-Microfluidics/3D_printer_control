...
class ManualControls:
    ...
    def __init__(self):
        ...
        self.position_log_file = str(
            Path.cwd() / "logs" / "calibration_position_log.txt"
        )
    ...
    def getPositionGalil(self):
        """Move the main Z stage. All units in mm"""
        message = {"position": self.galil.cntsToMm(self.galil.getPosition())}
    ...
    def lightEngineStop(self):
        ...
        socketio.emit(
            "light_engine_stop_complete", namespace="/calibrate", broadcast=True
        )
    ...
    def lightEngineProject(self, image, ledPower, repeat, exposure):
        ...
        if repeat:  # repeat == 1 means show once, == 0 means repeat forever
            self.projector.screenThread.screen.clear()
        socketio.emit(
            "light_engine_start_complete", namespace="/calibrate", broadcast=True
        )
