...
class Galil():
    ...
    def axisOn(self):
        self.send("SH")        # send the motor enable command

    # turn off the specified axis
    def axisOff(self):
        self.send("MO")        # send the motor off command

    # convert mm to counts for the specified axis
    def mmToCnts(self, mm):
        return int(mm * self.ctspmm)

    # convert counts to mm for the specified axis
    def cntsToMm(self, counts):
        return counts / self.ctspmm
    ...
    def relMove(self, axis, mm=None, cnts=None):
        ...
        if cnts is not None:                    # if counts has been calculated or supplied
            if axis == "C":
                ...
                # self.waitForMotionComplete(start_position - 8000) # block until motion is complete
                ...
                self.send("BG{}".format(axis))                          # begin motion
                self.g.GMotionComplete(axis)                            # block until motion is complete
                # self.waitForMotionComplete(cnts)                # block until motion is complete
    ...
    def absMove(self, axis, mm=None, cnts=None):
        ...
        if mm is not None:                      # if mm were supplied
            cnts = self.mmToCnts(mm)      # convert to counts
        if cnts is not None:                    # if counts has been calculated or supplied
            self.send("PA{}={}".format(axis, cnts - 8000))  # move down the distance you want, and then 1mm further
            self.send("BG{}".format(axis))                   # begin motion
            self.g.GMotionComplete(axis)                # block until motion is complete
            # self.waitForMotionComplete(cnts - 8000)
            self.send("PR{}={}".format(axis, 8000))        # move up 1mm
            self.send("BG{}".format(axis))                    # begin motion
            # self.g.GMotionComplete(axis)                # block until motion is complete
    ...
    def waitForMotionComplete(self, cnts):
        # self.send("MC{}".format(axis)                  # issue the MC command, this gets us close but not all the way
    ...
    def setSpeed(self, speed):
        self.speed = speed
        self.send("SP={}".format(speed*self.ctspmm))   # set speed (mm/sec)

    def setAcceleration(self, acceleration):
        self.acceleration = acceleration
        self.send("AC={}".format(acceleration*self.ctspmm))    # set acceleration
        self.send("DC={}".format(acceleration*self.ctspmm))    # set deceleration
        self.send("SD={}".format(acceleration*self.ctspmm))    # set switch deceleration

    # this only needs to be run once on power up. It will turn off motors though
    def configure(self):

        # set default motion parameters on all axes
        self.send("SP={}".format(self.speed*self.ctspmm))           # set speed (mm/sec)
        self.send("AC={}".format(self.acceleration*self.ctspmm))    # set acceleration
        self.send("DC={}".format(self.acceleration*self.ctspmm))    # set deceleration
        self.send("SD={}".format(self.acceleration*self.ctspmm))    # set switch deceleration
