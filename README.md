...
To run the dummy server to develop on, do the following: 
  ...
  * If you don't already have the `python3-tk` package installed, install it.
...
After this is all done, run the dummy server with `python $FLASK_APP`. (Make sure you are using Anaconda) (\*Note from Clayton: I got the dummy server to work without Anaconda.) 
