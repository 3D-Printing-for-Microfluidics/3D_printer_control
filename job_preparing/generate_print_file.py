...
slice_folder = "slices"                     # The folder the slices are located in 
normal_exp_time_ms = 400                    # Normal layer exposure time 
...
try:
    # Default output filename inclide the date and time of creation 
    ...
    with ZipFile(output_filename, 'x') as myzip:
        ...
        # Save png files in the slice_folder directory to a list
        slices = os.listdir(slice_folder)   

        # Iterate over the slices 
        for i in range(len(slices)):   
            curr_slice = slices[i]
            path_to_slice = os.path.join(this_directory, slice_folder, curr_slice)

            # Check to see if each slice is good 
            if not validate_slice(path_to_slice): 
                error_msg = "Bad slice provided: " + curr_slice + "\n"
            ...
            myzip.write(os.path.join(slice_folder, curr_slice))
    ...
    error_msg =  "The file '" + output_filename
